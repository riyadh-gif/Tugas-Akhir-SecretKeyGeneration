...
print('JUMLAH BIT HASIL BCH CODE ALICE %d'%len(bitalice))

# SAVING BCH
book=Workbook()
sheet1=book.add_sheet('HasilBCHalice')
sheet1.write(0,0,'Alice')
#sheet1.write(0,1,'Bob')
for i in range(1,len(bitalice)+1):
    sheet1.write(i,0,float(bitalice[i-1]))
    #sheet1.write(i,1,float(bitbob[i-1]))
book.save('Hasil_BCH_ALICE_I10_7030_DeepLearning_New97.xls')
book.save(TemporaryFile())
end4=time.time()
waktu_kuan = 0.6293494701385498
kgrbch=len(bitalice)*((waktu_kuan) + (end4 - start4))
print('KGR = %f'%kgrbch)
print('Waktu Proses BCH Code ALICE : ', end4 - start4)
