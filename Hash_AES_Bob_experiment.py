...
start5=time.time()
workbook = xlrd.open_workbook(r"files\Hasil_BCH_Bob.xls", on_demand=True)
worksheet = workbook.sheet_by_name("BCH Bob")
...
print("Panjang Input UnivHASH bob %d" % len(bbob))
...
for i in range(jumlahkey):
    mat1 = []
    aaa = []
    aaa = bbob[(i * ukuranhash): (ukuranhash * (i + 1))]
    print(
    ...
    for x in range(0, len(Hashtable)):
        ...
        mat1.append(int(row))
    key1.append(mat1)
    print("Jumlah KEY Bob Sekarang : ", len(key1))
...
with open(r"files\univhash_Bob_doss2.csv", "w", newline="") as fp:
...
end5 = time.time()
#kgruniv=len(bbob)*((endhash - starthash) + (end_jana-start_jana) + (end3 - start3))
#print('UNIVHASH KGR = %f'%(kgruniv))
print('UNIVHASH Panjang bit hasil Universal Hash bob = %d, bob= %d'%(len(ax),len(ax)))
...
with open(r"files\sudahujinist_Bob.csv", newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        indeks.append(row)
prindek = []
...
for j in range(len(valid_indeks)):
    ...
    print(f'Hasil hash bob Kunci-{valid_indeks[j] + 1} = {hex1[j]}')
...
key_bob_1 = keybyte
hex_key_bob_1 = key_bob_1.hex()
print(hex_key_bob_1)

# Save the hexadecimal key into a .txt file
with open(r'key\key_bob.txt', 'w') as txt_file:
    txt_file.write(hex_key_bob_1)

print("Hex key saved to key_bob.txt")
