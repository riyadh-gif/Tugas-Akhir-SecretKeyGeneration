...
with open("sudahujinist_Alice_I10_7030_New97.csv", newline="") as f:
    reader = csv.reader(f)
    for row in reader:
        indeks.append(row)
prindek = []
...
for kunci in range(len(indek)):
    if hex1[kunci] == hex2[kunci]:
        keybitt = ''.join(str(e) for e in key1[indek[kunci]])
...
end7=time.time()
time7=end7-start7
print('Waktu Proses Enkripsi : ', time7)
end=time.time()
#print('Waktu Proses Keseluruhan Sistem : ', end_aes - start1)